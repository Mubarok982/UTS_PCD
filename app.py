...
if uploaded_file:
    ...
    with st.expander("🔍 Tahapan Lengkap Preprocessing", expanded=True):
        ...
        st.image(data["compressed"], caption="2️⃣ Resize (256x256)", use_column_width=True)

        col1, col2 = st.columns(2)
        col1.image(data["gray"], caption="3️⃣ Grayscale", use_column_width=True)
        col2.image(data["binary"], caption="4️⃣ Binerisasi (Otsu)", use_column_width=True)

        col3, col4 = st.columns(2)
        col3.image(data["blurred"], caption="5️⃣ Gaussian Blur", use_column_width=True)
        col4.image(data["enhanced"], caption="6️⃣ Histogram Equalization", use_column_width=True)
